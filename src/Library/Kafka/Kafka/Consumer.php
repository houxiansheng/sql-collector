...
+------------------------------------------------------------------------------
*/
use USQL\Library\Kafka\Kafka\Protocol\Encoder;
use USQL\Library\Kafka\Kafka\Protocol\Fetch\Topic;
use USQL\Library\Kafka\Kafka\Protocol\Fetch\Helper\FreeStream;
use USQL\Library\Kafka\Kafka\Protocol\Fetch\Helper\Helper;
use USQL\Library\Kafka\Kafka\Protocol\Fetch\Helper\CommitOffset;
...
class Consumer
{
    ...
    public function fetch()
    {
        ...
        $fetch = new Topic($streams, $data);

        // register fetch helper
        $freeStream = new FreeStream($this->client);
        $freeStream->setStreams($streams);
        Helper::registerHelper('freeStream', $freeStream);

        // register partition commit offset
        $commitOffset = new CommitOffset($this->client);
        $commitOffset->setGroup($this->group);
        Helper::registerHelper('commitOffset', $commitOffset);

        $updateConsumer = new Consumer($this);
        Helper::registerHelper('updateConsumer', $updateConsumer);
