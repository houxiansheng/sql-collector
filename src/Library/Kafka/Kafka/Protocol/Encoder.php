...
namespace USQL\Library\Kafka\Kafka\Protocol;

use USQL\Library\Kafka\Kafka\Exception\NotSupported;
...
class Encoder extends Protocol
{
    ...
    public function produceRequest($payloads, $compression = self::COMPRESSION_NONE)
    {
        if (!isset($payloads['data'])) {
            throw new Protocol('given procude data invalid. `data` is undefined.');
        }
    ...
    public function metadataRequest($topics)
    {
        ...
        foreach ($topics as $topic) {
            if (!is_string($topic)) {
                throw new Protocol('request metadata topic array have invalid value. ');
            }
        }
    ...
    {
        if (!isset($payloads['data'])) {
            throw new Protocol('given fetch kafka data invalid. `data` is undefined.');
        }
        ...
        if (!isset($payloads['max_wait_time'])) {
            $payloads['max_wait_time'] = 10000; // default timeout 100ms
        }

        if (!isset($payloads['min_bytes'])) {
            //$payloads['min_bytes'] = 64 * 1024; // 64k
            $payloads['min_bytes'] = 1; // 64k
        }
    ...
    {
        if (!isset($payloads['data'])) {
            throw new Protocol('given offset data invalid. `data` is undefined.');
        }
    ...
    {
        if (!isset($payloads['data'])) {
            throw new Protocol('given commit offset data invalid. `data` is undefined.');
        }

        if (!isset($payloads['group_id'])) {
            throw new Protocol('given commit offset data invalid. `group_id` is undefined.');
        }
    ...
    {
        if (!isset($payloads['data'])) {
            throw new Protocol('given fetch offset data invalid. `data` is undefined.');
        }

        if (!isset($payloads['group_id'])) {
            throw new Protocol('given fetch offset data invalid. `group_id` is undefined.');
        }
    ...
    public static function encodeArray(array $array, $func, $options = null)
    {
        if (!is_callable($func, false)) {
            throw new Protocol('Encode array failed, given function is not callable.');
        }
    ...
    protected static function _encodeProcudePartion($values, $compression)
    {
        if (!isset($values['partition_id'])) {
            throw new Protocol('given produce data invalid. `partition_id` is undefined.');
        }

        if (!isset($values['messages']) || empty($values['messages'])) {
            throw new Protocol('given produce data invalid. `messages` is undefined.');
        }
    ...
    protected static function _encodeProcudeTopic($values, $compression)
    {
        if (!isset($values['topic_name'])) {
            throw new Protocol('given produce data invalid. `topic_name` is undefined.');
        }

        if (!isset($values['partitions']) || empty($values['partitions'])) {
            throw new Protocol('given produce data invalid. `partitions` is undefined.');
        }
    ...
    {
        if (!isset($values['partition_id'])) {
            throw new Protocol('given fetch data invalid. `partition_id` is undefined.');
        }
    ...
    {
        if (!isset($values['topic_name'])) {
            throw new Protocol('given fetch data invalid. `topic_name` is undefined.');
        }

        if (!isset($values['partitions']) || empty($values['partitions'])) {
            throw new Protocol('given fetch data invalid. `partitions` is undefined.');
        }
    ...
    {
        if (!isset($values['partition_id'])) {
            throw new Protocol('given offset data invalid. `partition_id` is undefined.');
        }
    ...
    {
        if (!isset($values['topic_name'])) {
            throw new Protocol('given offset data invalid. `topic_name` is undefined.');
        }

        if (!isset($values['partitions']) || empty($values['partitions'])) {
            throw new Protocol('given offset data invalid. `partitions` is undefined.');
        }
    ...
    {
        if (!isset($values['partition_id'])) {
            throw new Protocol('given commit offset data invalid. `partition_id` is undefined.');
        }

        if (!isset($values['offset'])) {
            throw new Protocol('given commit offset data invalid. `offset` is undefined.');
        }
    ...
    {
        if (!isset($values['topic_name'])) {
            throw new Protocol('given commit offset data invalid. `topic_name` is undefined.');
        }

        if (!isset($values['partitions']) || empty($values['partitions'])) {
            throw new Protocol('given commit offset data invalid. `partitions` is undefined.');
        }
    ...
    {
        if (!isset($values['partition_id'])) {
            throw new Protocol('given fetch offset data invalid. `partition_id` is undefined.');
        }
    ...
    {
        if (!isset($values['topic_name'])) {
            throw new Protocol('given fetch offset data invalid. `topic_name` is undefined.');
        }

        if (!isset($values['partitions']) || empty($values['partitions'])) {
            throw new Protocol('given fetch offset data invalid. `partitions` is undefined.');
        }
